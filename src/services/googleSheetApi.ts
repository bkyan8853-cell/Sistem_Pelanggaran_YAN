...
async function fetchWithAuth<T>(url: string, options: RequestInit = {}): Promise<T> {
  ...
  if (!response.ok) {
    ...
    let errMessage = `Gagal (HTTP ${response.status})`;
    try {
      const parsed = JSON.parse(text);
      errMessage = parsed.message || parsed.error || errMessage;
    } catch (_) {
      if (text) {
        errMessage = `${errMessage}: ${text.substring(0, 100)}`;
      }
    }
...
export const googleSheetApi = {
  ...
  async login(username: string, password: string, role: UserRole): Promise<{ success: boolean; token: string; user: User }> {
    ...
    if (!res.ok) {
      ...
      let errMessage = "Gagal masuk. Silakan periksa kredensial Anda.";
      try {
        const parsed = JSON.parse(text);
        if (parsed.message) {
        ...
        errMessage = `Gagal masuk (HTTP ${res.status}): ${text.substring(0, 80) || "Koneksi terputus atau server error"}`;
      }
      throw new Error(errMessage);
    }
