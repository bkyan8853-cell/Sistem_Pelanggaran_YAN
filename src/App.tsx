...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (currentUser && currentUser.nama) {
      let changed = false;
      if (currentUser.nama.includes("Endang") || currentUser.nama.includes("Setyawati") || currentUser.nama === "Dra. Endang Setyawati") {
        currentUser.nama = "Wali Kelas";
        changed = true;
      }
      if (currentUser.nama.includes("Filmayenti")) {
        currentUser.nama = "Iien Puspitasari, S.Pd";
        changed = true;
      }
      if (changed) {
    ...
    const handleAuthChange = () => {
      ...
      if (u && u.nama) {
        let changed = false;
        if (u.nama.includes("Endang") || u.nama.includes("Setyawati") || u.nama === "Dra. Endang Setyawati") {
          u.nama = "Wali Kelas";
          changed = true;
        }
        if (u.nama.includes("Filmayenti")) {
          u.nama = "Iien Puspitasari, S.Pd";
          changed = true;
        }
        if (changed) {
          ...
          <div className="p-6 border-b border-slate-800/80 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <span className="text-[9px] text-slate-400 font-bold uppercase tracking-wider block mt-0.5">SMP, SMA, SMK YAYASAN ALDIANA NUSANTARA</span>
              </div>
            </div>
