...
// Settings API
app.get(["/api/settings", "/settings"], authMiddleware, (req, res) => {
  res.json({ success: true, settings: db.settings });
});

app.post(["/api/settings", "/settings"], authMiddleware, async (req, res) => {
...
// Force Sync API
app.post(["/api/settings/sync", "/settings/sync"], authMiddleware, async (req, res) => {
...
// SISWA API
app.get(["/api/students", "/students"], authMiddleware, async (req, res) => {
  try {
...
app.post(["/api/students", "/students"], authMiddleware, async (req, res) => {
...
app.delete(["/api/students/:id", "/students/:id"], authMiddleware, async (req, res) => {
...
app.get(["/api/violations", "/violations"], authMiddleware, async (req, res) => {
  try {
...
app.post(["/api/violations", "/violations"], authMiddleware, async (req, res) => {
...
app.delete(["/api/violations/:id", "/violations/:id"], authMiddleware, async (req, res) => {
  const id = req.params.id;
  const user = (req as any).user as User;
...
app.get(["/api/records", "/records"], authMiddleware, async (req, res) => {
  try {
...
app.post(["/api/records", "/records"], authMiddleware, async (req, res) => {
...
app.delete(["/api/records/:id", "/records/:id"], authMiddleware, async (req, res) => {
  const id = req.params.id;
  const user = (req as any).user as User;
...
// PEMBINAAN API
app.get(["/api/guidance", "/guidance"], authMiddleware, async (req, res) => {
  try {
...
app.get(["/api/dashboard", "/dashboard"], authMiddleware, async (req, res) => {
  try {
...
// ================= REMISI API =================
app.get(["/api/remisi", "/remisi"], authMiddleware, async (req, res) => {
  try {
...
app.post(["/api/remisi", "/remisi"], authMiddleware, async (req, res) => {
...
app.delete(["/api/remisi/:id", "/remisi/:id"], authMiddleware, async (req, res) => {
  const { id } = req.params;
  const user = (req as any).user as User;
...
// ABSENSI API
app.get(["/api/attendance", "/attendance"], authMiddleware, async (req, res) => {
  try {
...
app.post(["/api/attendance", "/attendance"], authMiddleware, async (req, res) => {
  try {
    const user = (req as any).user as User;
    const body = req.body;
...
// Global Error Handler to catch any unhandled exceptions and return them in the response
...
if (process.env.NODE_ENV === "production" || process.env.VERCEL) {
  ...
  console.log("Serving compiled production assets and SPA fallback.");
}
